...
return new class extends Migration
{
    ...
    {
        Schema::table('donations', function (Blueprint $table) {
            $cols = array_filter(['saas_fee_amount', 'atc_fee_estimated_amount', 'net_estimated_to_foundation'], fn ($c) => Schema::hasColumn('donations', $c));
            if (!empty($cols)) {
        ...
        Schema::table('foundations', function (Blueprint $table) {
            $cols = array_filter(['saas_fee_card', 'saas_fee_qr', 'atc_fee_card_est', 'atc_fee_qr_est'], fn ($c) => Schema::hasColumn('foundations', $c));
            if (!empty($cols)) {
                $table->dropColumn($cols);
            }
        });
