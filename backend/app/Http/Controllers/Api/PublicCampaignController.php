...
{
    /**
     * Retorna el listado de campañas activas del tenant con protección de Caché distribuido (Redis).
    ...
    public function index(Request $request, string $subdomain): JsonResponse
    {
        ...
        $data = Cache::remember("tenant_{$tenant->id}_campaigns_index", 60, function () use ($tenant) {
    ...
    /**
     * Retorna la información completa de una campaña específica con protección de Caché distribuido (Redis).
    ...
    public function show(Request $request, string $subdomain, string $slug = 'default'): JsonResponse
    {
        ...
        $cacheKey = "tenant_{$tenant->id}_campaign_show_{$slug}";

        $data = Cache::remember($cacheKey, 60, function () use ($tenant, $slug) {
