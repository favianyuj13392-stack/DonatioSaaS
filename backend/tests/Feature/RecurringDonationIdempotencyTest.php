...
class RecurringDonationIdempotencyTest extends TestCase
{
    ...
    /**
     * Prueba que el scheduler despache los jobs a la cola 'billing'.
     */
    public function test_recurring_donations_scheduler_dispatches_jobs(): void
    {
        Queue::fake();
        ...
        Queue::assertPushedOn('billing', ProcessSubscriptionBillingJob::class, function ($job) use ($subscription, $today) {
            return $job->subscriptionId === $subscription->id && $job->billingDate === $today;
        });
    }

    /**
     * Prueba que el Job procese el cobro con clave de idempotencia determinista y actualice la fecha.
     */
    public function test_billing_job_executes_with_idempotency(): void
    {
        $tenant = Foundation::create([
            'name'            => 'Fundación Esperanza',
            'subdomain'       => 'esperanza',
